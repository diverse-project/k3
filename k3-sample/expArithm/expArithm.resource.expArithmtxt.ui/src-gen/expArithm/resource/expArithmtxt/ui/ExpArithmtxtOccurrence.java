 ...
 */
public class ExpArithmtxtOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "expArithm.resource.expArithmtxt.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "expArithm.resource.expArithmtxt.ui.occurences.declaration";
	...
	private expArithm.resource.expArithmtxt.ui.ExpArithmtxtTokenScanner tokenScanner;
	 ...
	 */
	public ExpArithmtxtOccurrence(expArithm.resource.expArithmtxt.IExpArithmtxtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, expArithm.resource.expArithmtxt.ui.ExpArithmtxtTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new expArithm.resource.expArithmtxt.mopp.ExpArithmtxtMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(expArithm.resource.expArithmtxt.ui.ExpArithmtxtBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(expArithm.resource.expArithmtxt.ui.ExpArithmtxtOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(expArithm.resource.expArithmtxt.ui.ExpArithmtxtOccurrence.DECLARATION_ANNOTATION_ID);
	}
