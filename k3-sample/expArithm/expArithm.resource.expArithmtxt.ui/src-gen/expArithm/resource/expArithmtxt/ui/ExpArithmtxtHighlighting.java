 ...
 */
public class ExpArithmtxtHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionHelper positionHelper = new expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionHelper();
	...
	private expArithm.resource.expArithmtxt.ui.ExpArithmtxtTokenScanner scanner;
	...
	private expArithm.resource.expArithmtxt.ui.ExpArithmtxtEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private expArithm.resource.expArithmtxt.ui.ExpArithmtxtOccurrence occurrence;
	private expArithm.resource.expArithmtxt.ui.ExpArithmtxtBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, expArithm.resource.expArithmtxt.IExpArithmtxtBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public ExpArithmtxtHighlighting(expArithm.resource.expArithmtxt.IExpArithmtxtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, expArithm.resource.expArithmtxt.ui.ExpArithmtxtColorManager colorManager, expArithm.resource.expArithmtxt.ui.ExpArithmtxtEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = expArithm.resource.expArithmtxt.ui.ExpArithmtxtUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new expArithm.resource.expArithmtxt.ui.ExpArithmtxtTokenScanner(textResource, colorManager);
		occurrence = new expArithm.resource.expArithmtxt.ui.ExpArithmtxtOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new expArithm.resource.expArithmtxt.ui.ExpArithmtxtBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(expArithm.resource.expArithmtxt.ui.ExpArithmtxtEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new expArithm.resource.expArithmtxt.ui.ExpArithmtxtEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
