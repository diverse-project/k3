 ...
 */
public class ExpArithmtxtBracketSet {
	...
	private java.util.ArrayList<expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair {
	 ...
	 */
	private class ClosingListener implements expArithm.resource.expArithmtxt.ui.IExpArithmtxtBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public ExpArithmtxtBracketSet(expArithm.resource.expArithmtxt.ui.ExpArithmtxtEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new expArithm.resource.expArithmtxt.mopp.ExpArithmtxtMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair>();
	 ...
	 */
	public expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair getBracketPair(String opening, String closing) {
		for (expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair bracketPair : bracketPairs) {
	...
	public expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair remove(String opening, String closing) {
		for (expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, expArithm.resource.expArithmtxt.ui.ExpArithmtxtPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
