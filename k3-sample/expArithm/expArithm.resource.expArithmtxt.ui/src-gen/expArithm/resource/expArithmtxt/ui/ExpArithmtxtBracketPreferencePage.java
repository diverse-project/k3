 ...
 * </ul>
 */
public class ExpArithmtxtBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new expArithm.resource.expArithmtxt.mopp.ExpArithmtxtMetaInformation().getSyntaxName();
	
	private expArithm.resource.expArithmtxt.ui.ExpArithmtxtBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public ExpArithmtxtBracketPreferencePage() {
		super();
		
		expArithm.resource.expArithmtxt.IExpArithmtxtMetaInformation metaInformation = new expArithm.resource.expArithmtxt.mopp.ExpArithmtxtMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(expArithm.resource.expArithmtxt.ui.ExpArithmtxtUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new expArithm.resource.expArithmtxt.ui.ExpArithmtxtBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					expArithm.resource.expArithmtxt.IExpArithmtxtBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + expArithm.resource.expArithmtxt.ui.ExpArithmtxtPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof expArithm.resource.expArithmtxt.ui.ExpArithmtxtEditor) {
			((expArithm.resource.expArithmtxt.ui.ExpArithmtxtEditor) editor).invalidateTextRepresentation();
		}
	}
