...
public class ExpArithmtxtDefaultNameProvider implements expArithm.resource.expArithmtxt.IExpArithmtxtNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = expArithm.resource.expArithmtxt.util.ExpArithmtxtEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
