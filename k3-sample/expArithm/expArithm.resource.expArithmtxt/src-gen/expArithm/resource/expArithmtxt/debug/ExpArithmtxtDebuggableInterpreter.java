 ...
 */
public class ExpArithmtxtDebuggableInterpreter<ResultType, ContextType> extends expArithm.resource.expArithmtxt.debug.AbstractExpArithmtxtDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private expArithm.resource.expArithmtxt.util.AbstractExpArithmtxtInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private expArithm.resource.expArithmtxt.IExpArithmtxtCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public ExpArithmtxtDebuggableInterpreter(expArithm.resource.expArithmtxt.util.AbstractExpArithmtxtInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new expArithm.resource.expArithmtxt.IExpArithmtxtInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = expArithm.resource.expArithmtxt.util.ExpArithmtxtStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public expArithm.resource.expArithmtxt.util.AbstractExpArithmtxtInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		expArithm.resource.expArithmtxt.IExpArithmtxtLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private expArithm.resource.expArithmtxt.IExpArithmtxtLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		expArithm.resource.expArithmtxt.IExpArithmtxtCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = expArithm.resource.expArithmtxt.util.ExpArithmtxtEObjectUtil.getDepth(current);
		stopCondition = new expArithm.resource.expArithmtxt.IExpArithmtxtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = expArithm.resource.expArithmtxt.util.ExpArithmtxtEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = expArithm.resource.expArithmtxt.util.ExpArithmtxtEObjectUtil.getDepth(current) - 1;
		stopCondition = new expArithm.resource.expArithmtxt.IExpArithmtxtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
