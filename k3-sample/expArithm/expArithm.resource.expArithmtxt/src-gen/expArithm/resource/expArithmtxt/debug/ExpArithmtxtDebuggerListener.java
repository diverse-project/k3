 ...
 * code).
 */
public class ExpArithmtxtDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private expArithm.resource.expArithmtxt.debug.AbstractExpArithmtxtDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, expArithm.resource.expArithmtxt.util.ExpArithmtxtPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, expArithm.resource.expArithmtxt.util.ExpArithmtxtPair<String, Object>>();
	...
	public ExpArithmtxtDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		expArithm.resource.expArithmtxt.debug.ExpArithmtxtDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = expArithm.resource.expArithmtxt.util.ExpArithmtxtStringUtil.encode('#', stack);
				expArithm.resource.expArithmtxt.debug.ExpArithmtxtDebugMessage message = new expArithm.resource.expArithmtxt.debug.ExpArithmtxtDebugMessage(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				expArithm.resource.expArithmtxt.debug.ExpArithmtxtDebugMessage message = new expArithm.resource.expArithmtxt.debug.ExpArithmtxtDebugMessage(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.GET_VARIABLES)) {
				...
				expArithm.resource.expArithmtxt.debug.ExpArithmtxtDebugMessage message = new expArithm.resource.expArithmtxt.debug.ExpArithmtxtDebugMessage(expArithm.resource.expArithmtxt.debug.EExpArithmtxtDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return expArithm.resource.expArithmtxt.util.ExpArithmtxtStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		expArithm.resource.expArithmtxt.util.ExpArithmtxtPair<String, Object> pair = new expArithm.resource.expArithmtxt.util.ExpArithmtxtPair<String, Object>(name, value);
	...
	public expArithm.resource.expArithmtxt.debug.AbstractExpArithmtxtDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(expArithm.resource.expArithmtxt.debug.AbstractExpArithmtxtDebuggable debuggable) {
