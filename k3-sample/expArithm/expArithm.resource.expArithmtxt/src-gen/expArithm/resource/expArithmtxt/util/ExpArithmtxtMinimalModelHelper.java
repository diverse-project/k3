 ...
 */
public class ExpArithmtxtMinimalModelHelper {
	
	private final static expArithm.resource.expArithmtxt.util.ExpArithmtxtEClassUtil eClassUtil = new expArithm.resource.expArithmtxt.util.ExpArithmtxtEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + expArithm.resource.expArithmtxt.util.ExpArithmtxtStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = expArithm.resource.expArithmtxt.util.ExpArithmtxtListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + expArithm.resource.expArithmtxt.util.ExpArithmtxtStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = expArithm.resource.expArithmtxt.util.ExpArithmtxtListUtil.castListUnchecked(value);
