 ...
 * contains
 * <code>expArithm.resource.expArithmtxt.IExpArithmtxtTextDiagnostic</code>s it
 * uses the more precise information of this extended diagnostic type.
 */
public class ExpArithmtxtMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = expArithm.resource.expArithmtxt.mopp.ExpArithmtxtPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object>> commands = new java.util.ArrayList<expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object>> commandsToProcess = new java.util.ArrayList<expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, expArithm.resource.expArithmtxt.IExpArithmtxtTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final expArithm.resource.expArithmtxt.IExpArithmtxtTextDiagnostic diagnostic) {
		final expArithm.resource.expArithmtxt.IExpArithmtxtProblem problem = diagnostic.getProblem();
		expArithm.resource.expArithmtxt.ExpArithmtxtEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == expArithm.resource.expArithmtxt.ExpArithmtxtEProblemSeverity.ERROR) {
					...
					expArithm.resource.expArithmtxt.IExpArithmtxtTextDiagnostic textDiagnostic = (expArithm.resource.expArithmtxt.IExpArithmtxtTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof expArithm.resource.expArithmtxt.mopp.ExpArithmtxtResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((expArithm.resource.expArithmtxt.mopp.ExpArithmtxtResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<expArithm.resource.expArithmtxt.IExpArithmtxtQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, expArithm.resource.expArithmtxt.util.ExpArithmtxtStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (expArithm.resource.expArithmtxt.ExpArithmtxtEProblemType nextType : expArithm.resource.expArithmtxt.ExpArithmtxtEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, expArithm.resource.expArithmtxt.ExpArithmtxtEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(expArithm.resource.expArithmtxt.ExpArithmtxtEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(expArithm.resource.expArithmtxt.ExpArithmtxtEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new expArithm.resource.expArithmtxt.util.ExpArithmtxtRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new expArithm.resource.expArithmtxt.IExpArithmtxtCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					expArithm.resource.expArithmtxt.mopp.ExpArithmtxtPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
