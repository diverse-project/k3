...
/**
 * The ExpArithmtxtTaskItemDetector is used to find task items in text documents.
 * The current implementation searches for specific keywords to detect task items.
 * The ExpArithmtxtTaskItemDetector is used both by the TaskItemBuilder and the
 * editor.
 */
public class ExpArithmtxtTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<expArithm.resource.expArithmtxt.mopp.ExpArithmtxtTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<expArithm.resource.expArithmtxt.mopp.ExpArithmtxtTaskItem> foundItems = new java.util.ArrayList<expArithm.resource.expArithmtxt.mopp.ExpArithmtxtTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new expArithm.resource.expArithmtxt.mopp.ExpArithmtxtTaskItem(keyword, message, localLine, offset, end));
