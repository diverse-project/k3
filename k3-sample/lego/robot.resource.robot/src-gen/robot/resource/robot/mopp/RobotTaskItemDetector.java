...
public class RobotTaskItemDetector {
	...
	public java.util.List<robot.resource.robot.mopp.RobotTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<robot.resource.robot.mopp.RobotTaskItem> foundItems = new java.util.ArrayList<robot.resource.robot.mopp.RobotTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new robot.resource.robot.mopp.RobotTaskItem(keyword, message, localLine, offset, end));
