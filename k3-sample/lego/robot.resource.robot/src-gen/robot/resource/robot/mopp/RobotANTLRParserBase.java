...
public abstract class RobotANTLRParserBase extends org.antlr.runtime3_4_0.Parser implements robot.resource.robot.IRobotTextParser {
	 ...
	 * result object.
	 */
	protected java.util.Collection<robot.resource.robot.IRobotCommand<robot.resource.robot.IRobotTextResource>> postParseCommands;
	 ...
	 */
	private robot.resource.robot.mopp.RobotTokenResolveResult tokenResolveResult = new robot.resource.robot.mopp.RobotTokenResolveResult();
	...
	protected void retrieveLayoutInformation(org.eclipse.emf.ecore.EObject element, robot.resource.robot.grammar.RobotSyntaxElement syntaxElement, Object object, boolean ignoreTokensAfterLastVisibleToken) {
		...
		isElementToStore |= syntaxElement instanceof robot.resource.robot.grammar.RobotPlaceholder;
		isElementToStore |= syntaxElement instanceof robot.resource.robot.grammar.RobotKeyword;
		isElementToStore |= syntaxElement instanceof robot.resource.robot.grammar.RobotEnumerationTerminal;
		isElementToStore |= syntaxElement instanceof robot.resource.robot.grammar.RobotBooleanTerminal;
		if (!isElementToStore) {
			return;
		}
		...
		layoutInformationAdapter.addLayoutInformation(new robot.resource.robot.mopp.RobotLayoutInformation(syntaxElement, object, offset, hiddenTokenText.toString(), visibleTokenText.toString()));
	...
	protected <ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> void registerContextDependentProxy(final robot.resource.robot.mopp.RobotContextDependentURIFragmentFactory<ContainerType, ReferenceType> factory, final ContainerType container, final org.eclipse.emf.ecore.EReference reference, final String id, final org.eclipse.emf.ecore.EObject proxy) {
		final int position;
		if (reference.isMany()) {
	...
	protected String formatTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = robot.resource.robot.util.RobotStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
	...
	public void setOptions(java.util.Map<?,?> options) {
		...
		if (this.options.containsKey(robot.resource.robot.IRobotOptions.DISABLE_LOCATION_MAP)) {
			this.disableLocationMap = true;
		}
		if (this.options.containsKey(robot.resource.robot.IRobotOptions.DISABLE_LAYOUT_INFORMATION_RECORDING)) {
	...
	protected void addMapEntry(org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EStructuralFeature structuralFeature, robot.resource.robot.mopp.RobotDummyEObject dummy) {
		Object value = element.eGet(structuralFeature);
		Object mapKey = dummy.getValueByName("key");
		Object mapValue = dummy.getValueByName("value");
		if (value instanceof org.eclipse.emf.common.util.EMap<?, ?>) {
			org.eclipse.emf.common.util.EMap<Object, Object> valueMap = robot.resource.robot.util.RobotMapUtil.castToEMap(value);
	...
	protected org.eclipse.emf.ecore.EObject apply(org.eclipse.emf.ecore.EObject target, java.util.List<org.eclipse.emf.ecore.EObject> dummyEObjects) {
		...
		for (org.eclipse.emf.ecore.EObject object : dummyEObjects) {
			assert(object instanceof robot.resource.robot.mopp.RobotDummyEObject);
			robot.resource.robot.mopp.RobotDummyEObject dummy = (robot.resource.robot.mopp.RobotDummyEObject) object;
	...
	protected robot.resource.robot.mopp.RobotTokenResolveResult getFreshTokenResolveResult() {
		tokenResolveResult.clear();
		return tokenResolveResult;
	}
	
	protected robot.resource.robot.mopp.RobotReferenceResolverSwitch getReferenceResolverSwitch() {
		robot.resource.robot.mopp.RobotReferenceResolverSwitch resolverSwitch = (robot.resource.robot.mopp.RobotReferenceResolverSwitch) metaInformation.getReferenceResolverSwitch();
