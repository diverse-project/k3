 ...
 * contains <code>robot.resource.robot.IRobotTextDiagnostic</code>s it uses the
...
public class RobotMarkerHelper {
	 ...
	 */
	public static final String MARKER_TYPE = robot.resource.robot.mopp.RobotPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<robot.resource.robot.IRobotCommand<Object>> commands = new java.util.ArrayList<robot.resource.robot.IRobotCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(robot.resource.robot.IRobotCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<robot.resource.robot.IRobotCommand<Object>> commandsToProcess = new java.util.ArrayList<robot.resource.robot.IRobotCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (robot.resource.robot.IRobotCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, robot.resource.robot.IRobotTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final robot.resource.robot.IRobotTextDiagnostic diagnostic) {
		final robot.resource.robot.IRobotProblem problem = diagnostic.getProblem();
		robot.resource.robot.RobotEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new robot.resource.robot.IRobotCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == robot.resource.robot.RobotEProblemSeverity.ERROR) {
					...
					robot.resource.robot.IRobotTextDiagnostic textDiagnostic = (robot.resource.robot.IRobotTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof robot.resource.robot.mopp.RobotResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((robot.resource.robot.mopp.RobotResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<robot.resource.robot.IRobotQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, robot.resource.robot.util.RobotStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (robot.resource.robot.RobotEProblemType nextType : robot.resource.robot.RobotEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, robot.resource.robot.RobotEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(robot.resource.robot.RobotEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(robot.resource.robot.RobotEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new robot.resource.robot.util.RobotRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new robot.resource.robot.IRobotCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					robot.resource.robot.mopp.RobotPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
