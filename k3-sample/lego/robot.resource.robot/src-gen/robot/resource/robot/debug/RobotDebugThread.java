...
public class RobotDebugThread extends robot.resource.robot.debug.RobotDebugElement implements org.eclipse.debug.core.model.IThread, robot.resource.robot.debug.IRobotDebugEventListener {
	
	private boolean suspended = false;
	private robot.resource.robot.debug.RobotDebugTarget debugTarget;
	
	public RobotDebugThread(robot.resource.robot.debug.RobotDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			robot.resource.robot.debug.RobotDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = robot.resource.robot.util.RobotStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new robot.resource.robot.debug.RobotStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public robot.resource.robot.debug.RobotDebugTarget getTarget() {
		return debugTarget;
	}
