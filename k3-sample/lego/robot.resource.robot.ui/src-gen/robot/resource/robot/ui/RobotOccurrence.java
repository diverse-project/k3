...
public class RobotOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "robot.resource.robot.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "robot.resource.robot.ui.occurences.declaration";
	...
	private robot.resource.robot.ui.RobotTokenScanner tokenScanner;
	 ...
	 */
	public RobotOccurrence(robot.resource.robot.IRobotTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, robot.resource.robot.ui.RobotTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new robot.resource.robot.mopp.RobotMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(robot.resource.robot.ui.RobotBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, robot.resource.robot.ui.RobotPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, robot.resource.robot.ui.RobotPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, robot.resource.robot.ui.RobotPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == robot.resource.robot.ui.RobotPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(robot.resource.robot.ui.RobotOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(robot.resource.robot.ui.RobotOccurrence.DECLARATION_ANNOTATION_ID);
	}
