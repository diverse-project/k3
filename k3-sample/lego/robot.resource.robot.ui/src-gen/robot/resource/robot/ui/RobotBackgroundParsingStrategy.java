...
public class RobotBackgroundParsingStrategy {
	 ...
	 */
	public void parse(org.eclipse.jface.text.DocumentEvent event, final robot.resource.robot.IRobotTextResource resource, final robot.resource.robot.ui.RobotEditor editor) {
	 ...
	 * Schedules a task for background parsing that will be started after a delay.
	 */
	public void parse(org.eclipse.jface.text.IDocument document, final robot.resource.robot.IRobotTextResource resource, final robot.resource.robot.ui.RobotEditor editor, long delay) {
	 ...
	 * Schedules a task for background parsing that will be started after a delay.
	 */
	public void parse(final String contents, final robot.resource.robot.IRobotTextResource resource, final robot.resource.robot.ui.RobotEditor editor, long delay) {
		if (resource == null) {
			return;
		}
		if (contents == null) {
			...
			while (newContents != null ) {
				while (newContents != null) {
					try {
						...
						if (resource instanceof robot.resource.robot.mopp.RobotResource) {
							robot.resource.robot.mopp.RobotResource concreteResource = (robot.resource.robot.mopp.RobotResource) resource;
