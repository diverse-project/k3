...
class AspectGenerator {
	...
	def static void main(String[] args) {
		...
		aspectGenerate(projectPath, projectName, "ASMLogo.ecore", "", "testbase", ".aspects", operationName,
			operationParams, listNewClass)

	}

	def static Context aspectGenerate(String projectPath, String projectName, String ecoreURI, String ecoreBasePackage,
		String aspectBasePackage, String aspectPackPostFix, String operationName, List<String> operationParams,
		List<String> listNewClass) {
		val Context context = new Context(projectPath, projectName, ecoreBasePackage, aspectBasePackage,
			aspectPackPostFix, operationName, listNewClass, operationParams)
...
class EPackageAspect {
	...
	def void generateAspect(Context context) {
		...
		if (_self.EClassifiers.exists[elt|elt instanceof EClass]) {
			...
			FileManager.writeFile(context.projectPath, _self.name + "Aspects",
				_self.getAspectPackageQualifiedName(context), context.kmtContent.toString)
	 ...
	 */
	def String getAspectPackageQualifiedName(Context context) {
		if (context.aspectBasePackage.isEmpty)
			return _self.getEPackageQualifiedName(context) + context.aspectPackagePostFix
		else
			return context.aspectBasePackage + "." + _self.getEPackageQualifiedName(context) +
				context.aspectPackagePostFix
	}

	def String getEcoreBasePackageQualifiedName(Context context) {
	...
	def String getEPackageQualifiedName(Context context) {
