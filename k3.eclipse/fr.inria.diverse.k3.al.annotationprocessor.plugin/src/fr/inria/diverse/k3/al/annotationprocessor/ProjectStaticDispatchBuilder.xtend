...
class ProjectStaticDispatchBuilder {
	 ...
	 * 
	 */
	def void writeTempStaticDispatchFile(CompilationUnit compilationUnit, extension CodeGenerationContext context) {
		...
		var Path targetFilePath = compilationUnit.filePath.projectFolder.append(
			"/" + STATICDISPATCH_GENFOLDER + "/" + destFileName + "." + INCREMENTALSTATICDISPATCH_FILEEXT)
	 ...
	 */
	def void cleanDeprecatedDispatchFiles(CompilationUnit compilationUnit,
		...
		val childWithCorrectExtension = folderPath.children.filter [ f |
			f.fileExtension !== null && f.fileExtension == INCREMENTALSTATICDISPATCH_FILEEXT
		]
		...
		childWithCorrectExtension.forEach [ f |
			...
			if (intermediate.endsWith("/xtend/" + INCREMENTALSTATICDISPATCH_FILEEXT)) {
				val srcXtendFile = intermediate.substring(0,
					intermediate.lastIndexOf("/xtend/" + INCREMENTALSTATICDISPATCH_FILEEXT)) + ".xtend"
	 ...
	 */
	def List<String> findExistingDispatchCalls(MutableMethodDeclaration methodDecl,
		...
		if (dispatchCodeForClass.empty) {
			...
			var Path folderPath = methodDecl.compilationUnit.filePath.projectFolder.append(
				"/" + STATICDISPATCH_GENFOLDER)
			val validChild = folderPath.children.filter [ f |
				f.fileExtension !== null && f.fileExtension == INCREMENTALSTATICDISPATCH_FILEEXT
			]
	 ...
	 */
	def void parseAndCacheDispatchStaticInjection(String content) {
