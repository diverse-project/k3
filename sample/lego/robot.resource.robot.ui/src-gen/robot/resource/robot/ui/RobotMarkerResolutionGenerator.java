	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(robot.resource.robot.IRobotOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof robot.resource.robot.mopp.RobotResource) {
					robot.resource.robot.mopp.RobotResource customResource = (robot.resource.robot.mopp.RobotResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<robot.resource.robot.IRobotQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final robot.resource.robot.IRobotQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									robot.resource.robot.ui.RobotUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			robot.resource.robot.ui.RobotUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<robot.resource.robot.IRobotQuickFix> getQuickFixes(robot.resource.robot.IRobotTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					robot.resource.robot.IRobotQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
