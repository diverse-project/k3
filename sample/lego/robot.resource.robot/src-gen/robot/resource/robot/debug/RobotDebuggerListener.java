...
public class RobotDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private robot.resource.robot.debug.AbstractRobotDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, robot.resource.robot.util.RobotPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, robot.resource.robot.util.RobotPair<String, Object>>();
	...
	private void runDebugger() throws java.io.IOException {
		...
		robot.resource.robot.debug.RobotDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = robot.resource.robot.util.RobotStringUtil.encode('#', stack);
				robot.resource.robot.debug.RobotDebugMessage message = new robot.resource.robot.debug.RobotDebugMessage(robot.resource.robot.debug.ERobotDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				robot.resource.robot.debug.RobotDebugMessage message = new robot.resource.robot.debug.RobotDebugMessage(robot.resource.robot.debug.ERobotDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(robot.resource.robot.debug.ERobotDebugMessageTypes.GET_VARIABLES)) {
				...
				robot.resource.robot.debug.RobotDebugMessage message = new robot.resource.robot.debug.RobotDebugMessage(robot.resource.robot.debug.ERobotDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return robot.resource.robot.util.RobotStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		robot.resource.robot.util.RobotPair<String, Object> pair = new robot.resource.robot.util.RobotPair<String, Object>(name, value);
	...
	public robot.resource.robot.debug.AbstractRobotDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(robot.resource.robot.debug.AbstractRobotDebuggable debuggable) {
